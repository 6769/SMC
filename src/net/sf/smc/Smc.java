...
//
    ...
    // Constructors.
    //
    private static final String VERSION = "v. 6.6.1";
...
//
    ...
    private static boolean parseArgs(final String[] args) {
        ...
        if (helpFlag == false) {
            ...
            if (retcode  && _targetLanguage == null) {
                retcode = false;
                _errorMsg = "Target language was not specified.\nUsage help for '-h,--help'";
            }
        }
    ...
    private static boolean _needHelp(final String[] args) {
        ...
        for (i = 0; i < args.length && retval == false; ++i) {
            if (args[i].startsWith("-h") ||args[i].startsWith("--help")) {
                retval = true;
                _usage(System.out);
            } else if (args[i].startsWith("-vers") ) {
                retval = true;
    ...
    private static boolean _isValidCast(final String castType) {
        return (castType.equals("dynamic_cast") ||
                castType.equals("static_cast")  ||
                castType.equals("reinterpret_cast") );
    ...
    private static void _generateCode(final SmcFSM fsm)
        ...
        if (_accessLevel == null) {
            _accessLevel = "public";
        } else if (_accessLevel.equals(PACKAGE_LEVEL) ) {
